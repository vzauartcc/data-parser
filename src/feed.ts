...
export async function fetchPilots(redis: Redis) {
	try {
		...
		const response = await fetch(`https://data.vatsim.net/v3/vatsim-data.json?t=${Date.now()}`, {
			method: 'GET',
			headers: {
				'Content-Type': 'application/json',
			},
...
export async function fetchControllers(redis: Redis) {
	try {
		...
		const redisControllers = redController && redController.length ? redController.split('|') : [];
...
export async function fetchPireps() {
	try {
		...
		const response = await fetch(
			'https://aviationweather.gov/api/data/pirep?id=KRFD&distance=200&age=2&format=json',
			{
				method: 'GET',
